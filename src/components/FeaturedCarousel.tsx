...
import { comparablePriceCOP, discountVsAveragePercent, fixAccents, formatCOP, isSenaName, ratingLabel, universityIndicatorScore } from "../lib/utils";

export default function FeaturedCarousel({
  title = "Excelentes ofertas del momento",
  items,
  avgByKey,
  viewAllHref = "#resultados"
}: {
  title?: string;
  items: ProgramHit[];
  ...
  return (
    <section className="mt-8">
      <div className="flex items-center justify-between gap-3">
        ...
        <a href={viewAllHref} className="text-sm font-semibold text-[#044AA9] hover:underline">
          Ver más ofertas →
        </a>
      </div>
      ...
      <div className="mt-4 relative">
        ...
        >
          {cards.map(({ h, score, label, discount }) => {
            ...
            const p = h.program;
            const sena = isSenaName(u.name);
            const priceText = sena ? "Gratis" : formatCOP(comparablePriceCOP(p));
            return (
              <div
                key={h.id}
                className="snap-start w-[260px] sm:w-[280px] flex-none rounded-2xl bg-white ring-1 ring-slate-200 overflow-hidden"
              >
                <div className="p-3">
